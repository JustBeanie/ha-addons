...
After its initial reconciliation, HA Docs also subscribes to Home Assistant automation and script entity changes. Saving one reloads just that entity, which Home Assistant publishes as a brief `unavailable` round trip — its registry entry outlives the platform entity, so the state goes `off`, `unavailable`, `off` inside a couple of milliseconds — and that round trip is what HA Docs watches for. A description is not an entity attribute, so there is nothing else in the event to see it by. Script execution state flips and last-run metadata are ignored. It debounces the burst, then runs the same report-only checker for just the entities that changed—never a new full scan. The log records `Entity update detected; checking only entity=...`, followed by that entity's targeted result. A burst is one check rather than one per entity, which matters on a Home Assistant restart: that adds every automation and script back at once, and the batch turns what would be one subprocess each into a single pass over them. A full reconciliation runs only at add-on start or when the documentation source changes, because a documentation change can affect more than one entity.

An issue belongs to the entity it names and is withdrawn when that entity stops existing. A deletion arrives on the same subscription: the targeted check finds no such entity and clears its Repair within the debounce. A scan can only ever visit entities that do exist, though, so a deletion that happened while the add-on was stopped would leave its issue open indefinitely. Every poll therefore begins with a sweep that reads the Repairs registry over the websocket API—the core REST API has no repairs endpoint—and withdraws any `ha_docs_link_*` issue whose automation or script is gone. The sweep runs ahead of the repository fetch and does not depend on it, so it keeps working while a broken anchor has the site frozen, and a registry it cannot read is logged and skipped rather than retried into a failure.
