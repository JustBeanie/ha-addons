describe('Server helper methods', () => {
    ...
    beforeEach(() => {
        ...
        jest.doMock('yaml', () => ({ parse: jest.fn(() => ({})) }));
    ...
    test('configure polling accepts and persists class-based polling config', async () => {
        const server = Object.create(Server.prototype);
        server.bacnetClient = {
        ...
        await server._configurePolling(req, res);

        expect(server.bacnetClient.saveConfig).toHaveBeenCalledWith(req.body);
        expect(res.payload).toEqual({});
    });
    ...
    test('configure polling rejects invalid payload', async () => {
        const server = Object.create(Server.prototype);
        server.bacnetClient = {
            saveConfig: jest.fn()
        };
    ...
    test('scanDevice validates payload, saves config, and handles failure', async () => {
        ...
        const invalidRes = createResponse();
        await server._scanDevice({ body: {}, query: {} }, invalidRes);
        expect(invalidRes.statusCode).toBe(400);

        const saveRes = createResponse();
        await server._scanDevice({
