...
describe('BacnetClient', () => {
    ...
    beforeEach(() => {
        ...
        process.env.NODE_CONFIG = JSON.stringify({
            bacnet: { maxSegments: 112, maxApdu: 5 },
            polling: {
                globalConcurrency: 1,
                objectConcurrency: 2,
    ...
    test('polling retries without PROP_UNITS when a device rejects that property', async () => {
        mockReadPropertyMultiple
            .mockImplementationOnce((_addr, requestArray, _opts, cb) => {
                expect(requestArray[0].properties.map((property) => property.id)).toEqual([85, 77, 117]);
                cb(new Error('BacnetError - Class:2 - Code:32'));
            })
            .mockImplementationOnce((_addr, requestArray, _opts, cb) => {
                expect(requestArray[0].properties.map((property) => property.id)).toEqual([85, 77]);
                const objectId = requestArray[0].objectId;
                cb(null, buildValueResponse(objectId.type, objectId.instance, 1, 'Binary State'));
            })
            .mockImplementationOnce((_addr, requestArray, _opts, cb) => {
                expect(requestArray[0].properties.map((property) => property.id)).toEqual([85, 77]);
                const objectId = requestArray[0].objectId;
                cb(null, buildValueResponse(objectId.type, objectId.instance, 0, 'Binary State'));
            });
        ...
        const result = await client._readObjectPresentValue('10.0.0.3', 3, 1);
        const cachedFallback = await client._readObjectPresentValue('10.0.0.3', 3, 1);

        expect(result.error).toBeNull();
        expect(cachedFallback.error).toBeNull();
        expect(mockReadPropertyMultiple).toHaveBeenCalledTimes(3);
        expect(logger.log).toHaveBeenCalledWith(
            'warn',
            '[Polling] Units property is not supported for 3/1; retrying without units.'
        );
        cleanup(client);
    });
    ...
    test('emits deviceFound on iAm and loads request options', async () => {
        ...
        expect(client.requestOptions).toEqual({ maxSegments: 112, maxApdu: 5 });
        expect(client._buildRequestOptions(8)).toEqual({ maxSegments: 112, maxApdu: 5, priority: 8 });
        cleanup(client);
    });

    test('ready waits for asynchronous config loading and initial registration', async () => {
        const { EventEmitter } = require('events');
        let releaseRegistration;
        runtimeState.upsertDeviceState.mockImplementationOnce(() => new Promise((resolve) => {
            releaseRegistration = resolve;
        }));
        class AsyncBacnetConfig extends EventEmitter {
            async load() {
                this.emit('configLoaded', {
                    device: { deviceId: 87, address: '10.0.0.87' },
                    polling: { class: 'normal', intervalMs: 5000 },
                    objects: [{ objectId: { type: 2, instance: 87 } }]
                });
            }
        }

        const { BacnetClient } = require('../src/bacnet_client');
        const client = new BacnetClient({ runtimeState, bacnetConfig: new AsyncBacnetConfig() });
        ...
        await Promise.resolve();
        expect(runtimeState.upsertDeviceState).toHaveBeenCalled();
        ...
        expect(client.deviceConfigs.has('87')).toBe(true);
        cleanup(client);
    });
    ...
    test('configureSchedule cancels existing cron job and cron schedule sets due flag', async () => {
        ...
        await client.startPolling(
            { deviceId: 90, address: '10.0.0.90' },
            [{ objectId: { type: 2, instance: 1 } }],
            { intervalMs: 5000, class: 'fast' }
        );

        expect(cancel).toHaveBeenCalled();
        expect(runtime.schedule).toBeNull();
        expect(runtime.pollClass).toBe('fast');
        expect(runtime.polling).toEqual(expect.objectContaining({
            schedule: null,
            class: 'fast',
            intervalMs: 5000
        }));
    ...
    test('close cancels configured cron jobs', async () => {
        const cancel = jest.fn();
        mockScheduleJob.mockReturnValueOnce({ cancel });
        const { BacnetClient } = require('../src/bacnet_client');
        const client = new BacnetClient({ runtimeState, bacnetConfig });
        await client.ready;

        await client.startPolling(
            { deviceId: 91, address: '10.0.0.91' },
            [{ objectId: { type: 2, instance: 1 } }],
            { schedule: '*/5 * * * * *', class: 'normal' }
        );
        await client.close();
        ...
        expect(client.schedules.size).toBe(0);
    ...
    test('scheduler does not overlap polls for the same device', async () => {
        const { BacnetClient } = require('../src/bacnet_client');
        const client = new BacnetClient({ runtimeState, bacnetConfig });
        await client.ready;
        await client.startPolling(
            { deviceId: 92, address: '10.0.0.92' },
            [{ objectId: { type: 2, instance: 1 } }],
            { intervalMs: 1, class: 'fast' }
        );
        ...
        const pollSpy = jest.spyOn(client, '_pollDevice').mockReturnValue(poll);

        await client._schedulerLoop();
        expect(pollSpy).toHaveBeenCalledTimes(1);
        expect(client.inFlightDevices.has('92')).toBe(true);

        client.deviceRuntime.get('92').nextDueAt = Date.now();
        await client._schedulerLoop();
        ...
        await Promise.resolve();
        await Promise.resolve();
        expect(client.inFlightDevices.has('92')).toBe(false);
        cleanup(client);
    });
    ...
    test('scanDevice bounds object metadata reads by objectConcurrency', async () => {
        const objectIds = Array.from({ length: 6 }, (_value, instance) => ({ type: 2, instance }));
        const { BacnetClient } = require('../src/bacnet_client');
        const client = new BacnetClient({ runtimeState, bacnetConfig });
        await client.ready;
        jest.spyOn(client, '_readObjectListWithRetry').mockResolvedValue(buildObjectListResponse(objectIds));
        jest.spyOn(client, '_readObjectListCount').mockResolvedValue(objectIds.length);
        ...
        jest.spyOn(client, '_readObjectDiscoveryDetails').mockImplementation(async (_address, objectId) => {
            activeReads += 1;
            maxActiveReads = Math.max(maxActiveReads, activeReads);
            await readGate;
            activeReads -= 1;
            return { error: null, value: buildFullObjectResponse(objectId, `Object ${objectId.instance}`) };
        });

        const scan = client.scanDevice({ address: '10.0.0.6', deviceId: 6 });
        await Promise.resolve();
        await Promise.resolve();
        await Promise.resolve();
        expect(client._readObjectDiscoveryDetails).toHaveBeenCalledTimes(client.objectConcurrency);

        releaseReads();
        const objects = await scan;

        expect(objects).toHaveLength(objectIds.length);
        expect(maxActiveReads).toBe(client.objectConcurrency);
        cleanup(client);
    });
