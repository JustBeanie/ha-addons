...
describe('web admin app', () => {
        ...
        const context = {
            ...
            window: {
                ...
                location: {
                    ...
                    search: '',
                    pathname: '/api/hassio_ingress/token/admin/'
                }
            },
            setInterval: jest.fn(),
            setTimeout: jest.fn((fn) => fn()),
    ...
    test('index.html references Vue 3 and runtime navigation', () => {
        ...
        expect(html).toContain('<script src="vendor/vue.global.prod.js"></script>');
    ...
    test('uses the Home Assistant ingress prefix as the API base URL', () => {
        const { context } = loadAdminScript();

        expect(context.axios.defaults.baseURL).toBe('/api/hassio_ingress/token/');
    });
    ...
    test('device scan saves selected objects as a persistent polling configuration', async () => {
        const { context, exports } = loadAdminScript();
        const DeviceScan = exports.appOptions.components.DeviceScan;
        const component = {
            ...DeviceScan.data(),
        ...
        component.deviceId = 'Gree VRF/1';
        component.address = '192.168.1.20';
        component.pollClass = 'fast';
        component.intervalMs = '2500';
        component.objects = [
            {
        ...
        context.axios.put.mockResolvedValue({ data: {} });

        await component.saveForPolling();

        expect(context.axios.put).toHaveBeenCalledWith(
            '/api/bacnet/Gree%20VRF%2F1/config',
            expect.objectContaining({
                device: { deviceId: 'Gree VRF/1', address: '192.168.1.20' },
                polling: { class: 'fast', intervalMs: 2500 },
        ...
        expect(component.saveSuccess).toBe('Saved 1 object for polling.');
        expect(component.configuredObjectKeys.has('2_202')).toBe(true);
    });
});
